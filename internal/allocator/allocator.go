...
func (a *Allocator) storeAllocation(ctx context.Context, callSID, podName, sourcePool, merchantID string, allocatedAt time.Time) error {
	...
	// Track call in the pod's active calls SET.
	// This allows crash cleanup to find ALL active calls on a pod (important
	// for shared pods that handle multiple concurrent calls).
	podCallsKey := podInfoPrefix + podName + ":calls"
	if err := a.redis.SAdd(ctx, podCallsKey, callSID).Err(); err != nil {
		a.logger.Warn("failed to add call to pod calls set",
			zap.Error(err), zap.String("pod_name", podName), zap.String("call_sid", callSID))
	}
